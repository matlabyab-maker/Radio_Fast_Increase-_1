...
public class StationAdapter extends BaseAdapter {
    ...
    @Override public View getView(int p, View convert, ViewGroup parent){
        ...
        LinearLayout row=new LinearLayout(context); row.setOrientation(LinearLayout.HORIZONTAL); row.setGravity(Gravity.CENTER_VERTICAL); row.setPadding(8,4,4,4); row.setBackgroundColor(Color.rgb(230,222,209));
        ...
        TextView t=new TextView(context); t.setText(s.toString()); t.setTextColor(Color.rgb(16,42,67)); t.setTextSize(14); t.setGravity(Gravity.CENTER_VERTICAL); row.addView(t,new LinearLayout.LayoutParams(0,58,1));
        Button open=new Button(context); open.setBackgroundResource(com.fast.radio.R.drawable.radio_button_selector); open.setText("↗"); open.setTextSize(18); open.setContentDescription("Open stream in another player"); row.addView(open,new LinearLayout.LayoutParams(54,54));
        Button star=new Button(context); star.setBackgroundResource(com.fast.radio.R.drawable.radio_button_selector); star.setSelected(s.favorite); star.setText(s.favorite?"★":"☆"); star.setTextSize(18); star.setTextColor(Color.rgb(80,190,255)); row.addView(star,new LinearLayout.LayoutParams(54,54));
        ...
        row.setOnClickListener(v->listener.select(s));
        open.setOnClickListener(v->{ v.setSelected(true);
            if(s.url==null || s.url.trim().isEmpty()) return;
            try {
                Intent i=new Intent(Intent.ACTION_VIEW, Uri.parse(s.url.trim()));
                i.setType("audio/*");
                i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                context.startActivity(Intent.createChooser(i, "پخش با..."));
            } catch(Exception ignored) {}
        });
        star.setOnClickListener(v->{listener.favorite(s); v.setSelected(s.favorite); notifyDataSetChanged();});
        return row;
    }
}
